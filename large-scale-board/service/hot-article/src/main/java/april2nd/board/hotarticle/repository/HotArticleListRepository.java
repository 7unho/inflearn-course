...
public class HotArticleListRepository {
    ...
    public void add(Long articleId, LocalDateTime now, Long score, Long limit, Duration ttl) {
        redisTemplate.executePipelined((RedisCallback<?>) action -> {
            StringRedisConnection conn = (StringRedisConnection) action;
            String key = generateKey(now);
    ...
    private String generateKey(LocalDateTime now) {
        return generateKey(TIME_FORMATTER.format(now));
    }
